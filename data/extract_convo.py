...
def process_convo(convo_dir: str, encode: str="latin-1", decode: str="utf-8") -> tuple[pd.DataFrame, str, str]:
    ...
    for jfile_path in glob.glob(os.path.join(convo_dir, "message_*.json")):
        ...
        df_messages_loc = fix_encoding(df_messages_loc, ["sender_name", "content"], encode=encode, decode=decode)        
    ...
    df_engagement = df_message.groupby("sender_name").agg("count").sort_values("content").drop("timestamp_ms", axis="columns")
